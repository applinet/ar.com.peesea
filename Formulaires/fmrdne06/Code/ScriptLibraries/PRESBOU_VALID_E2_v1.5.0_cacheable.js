...
PRESBOU.actions.submit = function() {
	...
	var strTmp = $("#PRESBOU_AvailDate").val().trim();
	if(strTmp == "") { DisplayErrMsgOnField("PRESBOU_AvailDate", l_Forms_Err_ChampObligatoire + "\"" + l_PRESBOU_AvailDate + "\" !"); return false; }
	var DateFormat = /^(([3][0-1])|([0-2][0-9]))\/(([1][0-2])|([0][0-9]))\/((19)|(20))[0-9][0-9]$/;
	if(!DateFormat.test(strTmp)) { DisplayErrMsgOnField("PRESBOU_AvailDate", l_Forms_Err_DateFormat); return false; }
	...
	if(dateStart.getTime() > dateNow.getTime()) { DisplayErrMsgOnField("PRESBOU_AvailDate", l_PRESBOU_AvailDate_ErrWrongDate); return false; }


	// VIS
	var strTmp = $("#PRESBOU_VIS").val().trim(); 
	if(strTmp == "") { DisplayErrMsgOnField("PRESBOU_VIS", l_Forms_Err_ChampObligatoire + "\"" + l_PRESBOU_VIS + "\" !"); return false; }
	var ereg = /^[a-z0-9]{8}$/gi;
	if(!ereg.test(strTmp)) { DisplayErrMsgOnField("PRESBOU_VIS", l_PRESBOU_VIS_ErrWrongFormat); return false; }

	// Soumission
	fmr.document.submit();
};

// Calendrier "Date de mise à disposition"
function PRESBOU_AvailDate_DatePicker(nodeThis) {

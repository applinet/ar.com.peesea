var scanup = {
    ...
    sortTable: function(numCol, asc) {
		...
		arrTemp.sort(function(a, b) {
			...
			var argRowA = a.getElementsByTagName("td")[numCol].textContent || a.getElementsByTagName("td")[numCol].innerText;
			var argRowB = b.getElementsByTagName("td")[numCol].textContent || b.getElementsByTagName("td")[numCol].innerText;
			// Tri sur la colonne
			if(argRowA > argRowB) {
				return (asc ? 1 : -1);
			}
			else if(argRowA < argRowB) {
				return (asc ? -1 : 1);
			}
			// Valeur identique
			else {
				// Tri secondaire
				argRowA = a.getElementsByTagName("td")[6].textContent || a.getElementsByTagName("td")[6].innerText;
				argRowB = b.getElementsByTagName("td")[6].textContent || b.getElementsByTagName("td")[6].innerText;
				if(argRowA > argRowB) {
					return (asc ? 1 : -1);
				}
				else if(argRowA < argRowB) {
